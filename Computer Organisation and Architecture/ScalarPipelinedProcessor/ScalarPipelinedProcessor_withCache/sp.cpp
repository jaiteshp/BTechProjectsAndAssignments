	...
	void pipeline()
	{
		...
		while(1)
		{
			...
			if((cycles != 1) && (!skipDecode))
			{
				...
				{
					if(!(regOcc[fr]) && !(regOcc[sr]) && !(regOcc[tr]))
					{
						regOcc[fr] = regOcc[sr] = regOcc[tr] = 4;
		...
		ofstream write_D$;
		write_D$.open("DCache_output.txt");

		for(int i = 0; i < 256; i++)
		{
			write_D$ << (D$[i] % (1 << 8)) << endl;
		}
		...
		write_Output << "Total no of instructions is " << instructions << endl; 

		for(int i = 0; i < 16; i++)
		{
			write_Output << "No of instructions of opcode " << i << " is " << instructionsType[i] << endl;
		}

		write_Output << "No of cycles is " << cycles << endl;
		write_Output << "CPI is " << ((float) cycles/instructions) << endl;

		write_Output << "No of stalls is " << stalls.size() << endl;

		for(int i = 0; i < stalls.size(); i++)
		{
			if(stalls[i])	write_Output << i << "th stall is RAW Hazard" << endl;
			else	write_Output << i << "th stall is Control Hazard" << endl;
		}
