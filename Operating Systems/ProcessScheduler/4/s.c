...
void startNext()
{
	...
	for(i = 1; i < 2 * nproc; i++)
	{
		...
		if(w_state[j] == 0)//just created
		{

		}
		else if(w_state[j] == 1)//ready
		{
			kill(SIGUSR2, w_pid[j]); //s made w run
			w_state[j] = 3;
		    gettimeofday(&t,NULL);
			addNumber(0, (float)((float)(t.tv_sec - t00.tv_sec) + ((float)(t.tv_usec - t00.tv_usec))/1000000), -1);
			curr_job = j;
		...
		else if(w_state[j] == 2)//waiting
		{
			//do nothing(search next)
		}
		else if(w_state[j] == 3)//running
		{
			//not possible
		}
		else if(w_state[j] == 4)//stopped
		{
			kill(SIGCONT, w_pid[j]); //unstop it
			w_state[j] = 3;
		    gettimeofday(&t,NULL);
			addNumber(0, (float)((float)(t.tv_sec - t00.tv_sec) + ((float)(t.tv_usec - t00.tv_usec))/1000000), -1);
			curr_job = j;
	...
	if(count == 0)
	{
		printf("nothing found for rr scheduler s 195\n");
	}
}
	...
	while(1)
    	{
    		...
    		if(noRunning())
    		{
    			//try to run the pointed job
    			//int newRow = findNext();
    			startNext();
    		}
    		else
    		{
    			int reqJobRow = findRunning(); 
    			kill(w_pid[reqJobRow], SIGSTOP);gettimeofday(&t,NULL);
				addNumber(1, (float)((float)(t.tv_sec - t00.tv_sec) + ((float)(t.tv_usec - t00.tv_usec))/1000000), -1);
			    addNumber(2, (float) reqJobRow, reqJobRow);
    			w_state[reqJobRow] = 4;
    			//try to run the pointed job;
    			startNext();
    		}
